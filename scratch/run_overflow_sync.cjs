...
async function testOverflow() {
  ...
  try {
    const listRes = await fetch('http://localhost:9998/json');
    ...
    ws.on('message', (data) => {
      ...
      if (msg.id === 10) {
        ...
        const resultPath = 'C:\\Users\\luanl\\.gemini\\antigravity\\brain\\6b67d4be-69d0-4a17-97cc-0a4fb66f39dd\\overflow_audit_result.json';
        fs.writeFileSync(resultPath, JSON.stringify(result, null, 2));
        console.log("OVERFLOW AUDIT COMPLETE. SAVED TO:", resultPath);
        ws.close();
        chromeProc.kill();
        process.exit(0);
      }
