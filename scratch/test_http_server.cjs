...
server.listen(5555, () => {
  console.log("Static HTTP server running on http://localhost:5555");
  ...
  setTimeout(() => {
    ...
    const cmd = `"${chromePath}" --headless=new --disable-gpu --window-size=390,844 --screenshot="${outputPath}" "http://localhost:5555/"`;
