...
// Read dynamically inside each function call so dotenv.config() in server.mjs
// has time to populate process.env before these values are accessed.
function getMetaPixelId() { return process.env.META_PIXEL_ID || ''2645703275845738''; }
function getMetaCapiToken() { return process.env.META_CAPI_ACCESS_TOKEN || ''''; }
function getMetaTestEventCode() { return process.env.META_TEST_EVENT_CODE || ''''; }
...
export async function sendMetaCapiEvent(eventName, eventId, order, req) {
  try {
    const META_CAPI_ACCESS_TOKEN = getMetaCapiToken();
      ...
      data: [
        {
          ...
          event_source_url: req ? (req.headers[''referer''] || ''https://miraclebrasil.com'') : ''https://miraclebrasil.com'',
          user_data: userData,
          custom_data: {
            currency: ''BRL'',
